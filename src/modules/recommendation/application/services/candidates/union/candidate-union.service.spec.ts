...
import { CandidateUnion } from '@/modules/recommendation/application/services/candidates/union/candidate-union.service';
...
describe('CandidateUnion', () => {
    ...
    it('preserves every relation contribution for one product', () => {
        ...
        expect(
            candidate?.contributions?.map((item) => item.relationType),
        ).toEqual(['CO_PURCHASE', 'CO_CART']);
    });
