...
function validateMetadata(
    ...
    for (const key of allowedKeys) {
        const currentValue = optionalString(
            metadata[key],
            `metadata.${key}`,
            128,
        );
...
export function validateInteractionEvent(
    ...
    if (!interactionTypeSet.has(interactionType as never)) {
        throw new InvalidInteractionEventError(
            'interactionType is not supported',
        );
    }
    ...
    if (Number.isNaN(Date.parse(occurredAt))) {
        throw new InvalidInteractionEventError(
            'occurredAt must be an ISO date',
        );
    }
    ...
    if (
        ...
        !['HYBRID', 'ML_HYBRID'].includes(String(recommendationRankingMode))
    ) {
        throw new InvalidInteractionEventError(
    ...
    return {
        ...
        data: {
            ...
            position:
                position === null || position === undefined ? null : position,
            quantity:
