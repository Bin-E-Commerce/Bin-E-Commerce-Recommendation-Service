...
import { validatePurchaseEvent } from '@/kafka/consumers/validators/event.validators';
                ...
                eachMessage: async ({ topic, partition, message }) => {
                    ...
                    await this.projectWithRetry(topic, event, async () => {
                        if (
                            topic === RECOMMENDATION_INTERACTIONS_TOPIC &&
                            event.eventName ===
                                RecommendationEvents.INTERACTION_RECORDED
                        ) {
                        ...
                        if (
                            (
                                RECOMMENDATION_PURCHASE_TOPICS as readonly string[]
                            ).includes(topic)
                        ) {
                ...
                if (attempt === maxAttempts) {
                    await this.producer.publish(
                        ...
                        event.eventId,
                        {
                            ...
                            eventName:
                                'recommendation.relation_projection_failed',
                            failedAt: new Date().toISOString(),
                            sourceTopic: topic,
