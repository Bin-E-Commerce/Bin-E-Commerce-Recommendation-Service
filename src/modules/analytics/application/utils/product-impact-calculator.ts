...
import type { ProductImpactEventRow } from "../../infrastructure/product-impact.repository";
import type { ProductImpactComparisonDto } from "../../presentation/dto/product-impact.dto";
import type {
  ProductImpactDaily,
  ProductImpactItem,
  ProductImpactMetric,
} from "../types/product-impact.types";
import {
  createProductImpactDateRange,
  parseProductImpactTimestamp,
} from "./product-impact-time.util";
...
function buildBeforeMetric(
  ...
  const baselineRows = rows.filter((row) => {
    const occurredAt = parseProductImpactTimestamp(row.occurredAt).getTime();
...
function buildAfterMetric(
  ...
  return {
    metric: {
      ...
      averageViews: Number((views / Math.max(1, daily.length)).toFixed(2)),
      averageSales: Number((sales / Math.max(1, daily.length)).toFixed(2)),
    },
    daily,
  };
}
