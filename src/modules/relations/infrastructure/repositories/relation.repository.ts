...
export class RelationRepository {
    ...
    async findRecentSignals(
        ...
        Array<{ eventId: string; productId: string; occurredAt: Date }>
    > {
    ...
    async addRelations(
        ...
        for (const input of inputs) {
            ...
            if (input.signalAt > current.signalAt)
                current.signalAt = input.signalAt;
    ...
    > {
        ...
        const halfLifeDays =
            Number.isFinite(configuredHalfLifeDays) &&
            configuredHalfLifeDays > 0
