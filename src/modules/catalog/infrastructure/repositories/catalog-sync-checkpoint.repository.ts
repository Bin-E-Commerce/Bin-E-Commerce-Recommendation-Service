...
export class CatalogSyncCheckpointRepository {
    constructor(
        @InjectRepository(RecommendationCatalogSyncCheckpointEntity)
        private readonly repository: Repository<RecommendationCatalogSyncCheckpointEntity>,
    ) {}

    // Đọc page kế tiếp, mặc định bắt đầu từ page một khi chưa có checkpoint.
    async getNextPage(syncName: string): Promise<number> {
        return (
            (await this.repository.findOne({ where: { syncName } }))
                ?.nextPage ?? 1
        );
    }
