...
export class CreateRecommendationRelationSignals1788031000000 implements MigrationInterface {
    ...
    async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `DROP INDEX IF EXISTS "IDX_recommendation_relation_signals_occurred"`,
        );
        await queryRunner.query(
            `DROP INDEX IF EXISTS "IDX_recommendation_relation_signals_product_occurred"`,
        );
        await queryRunner.query(
            `DROP INDEX IF EXISTS "IDX_recommendation_relation_signals_user_occurred"`,
        );
        await queryRunner.query(
            `DROP INDEX IF EXISTS "IDX_recommendation_relation_signals_session_occurred"`,
        );
        await queryRunner.query(
            `DROP TABLE IF EXISTS "recommendation_relation_signals"`,
        );
    }
}
