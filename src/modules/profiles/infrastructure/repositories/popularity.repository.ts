...
export class PopularityRepository {
  ...
  async incrementInteraction(
    input: { productId: string; views: number; clicks: number; cartAdds: number },
    ...
    await executor.query(
      `INSERT INTO recommendation_product_popularity_daily (product_id, bucket_date, views, clicks, cart_adds)
       ...
       DO UPDATE SET views = recommendation_product_popularity_daily.views + $2,
                     clicks = recommendation_product_popularity_daily.clicks + $3,
                     cart_adds = recommendation_product_popularity_daily.cart_adds + $4`,
    ...
    await executor.query(
      `INSERT INTO recommendation_product_popularity (product_id, views_1d, views_7d, clicks_1d, cart_adds_7d, popularity_score)
       VALUES ($1, $2, $2, $3, $4, $2 * 0.1 + $3 * 0.5 + $4 * 2)
       ON CONFLICT (product_id)
       DO UPDATE SET views_1d = recommendation_product_popularity.views_1d + $2,
                     views_7d = recommendation_product_popularity.views_7d + $2,
                     clicks_1d = recommendation_product_popularity.clicks_1d + $3,
                     cart_adds_7d = recommendation_product_popularity.cart_adds_7d + $4,
                     popularity_score = recommendation_product_popularity.popularity_score + ($2 * 0.1 + $3 * 0.5 + $4 * 2),
                     calculated_at = now()`,
      [input.productId, input.views, input.clicks, input.cartAdds],
    );
  }
  ...
  async incrementPurchase(
    ...
    await executor.query(
      `INSERT INTO recommendation_product_popularity_daily (product_id, bucket_date, purchases)
       ...
       ON CONFLICT (product_id, bucket_date)
       DO UPDATE SET purchases = GREATEST(0, recommendation_product_popularity_daily.purchases + $2)`,
    ...
    await executor.query(
      `INSERT INTO recommendation_product_popularity (product_id, purchases_30d, popularity_score)
       VALUES ($1, $2, $2 * 3)
       ON CONFLICT (product_id)
       DO UPDATE SET purchases_30d = GREATEST(0, recommendation_product_popularity.purchases_30d + $2),
                     popularity_score = GREATEST(0, recommendation_product_popularity.popularity_score + ($2 * 3)),
                     calculated_at = now()`,
      [input.productId, signedQuantity],
    );
  }
