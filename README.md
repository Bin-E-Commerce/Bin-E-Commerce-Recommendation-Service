<div align="center">
  ...
  Help each customer find the next product that makes sense, using intent, context, catalog intelligence, and safe AI rollout.
...
13. [Experiment assignment and rollout](#13-experiment-assignment-and-rollout)
...
The service keeps Standard Ranking as a deterministic baseline. Semantic candidates, co-behavior candidates, and AI scoring are optional layers that can fail or be rolled out gradually without taking recommendation serving offline.
...
- a guarantee that AI is used on every request.
...
| Ranking | Standard deterministic score and optional ML blend |
| Attribution | Sign and validate recommendation tracking context |
| Analytics | Aggregate impressions, clicks, conversions, and experiment data |
...
| Recommendation admin with analytics permission | Read overview, actors, activity, and experiments |
...
When AI is not eligible, unavailable, timed out, or returns a fallback model, the response remains HYBRID and does not claim ML treatment.
...
RANKING_EXPERIMENT_TRAFFIC_PERCENT=0
~~~

This keeps Standard Ranking as the only serving mode until the AI model is verified.
...
Standard Ranking is the always-available deterministic baseline. It is the fallback when AI is disabled, not ready, timed out, invalid, or configured with zero traffic.
...
AI scoring is eligible only when:

- ML_RANKING_ENABLED or the active runtime policy enables ML;
- the experiment is enabled;
- the actor is assigned to ML traffic;
- the AI Service is reachable;
- the model status is ready;
- the response contains valid, bounded scores for the requested candidates.
...
A fallback request must not be counted as an ML treatment in attribution analytics.

---

## 13. Experiment assignment and rollout

### Deterministic assignment

RankingExperimentService hashes:

- experiment ID;
- actor type;
- actor ID.

The same user or session remains in the same variant across requests. The hash is for stable assignment, not authentication or authorization.
...
| CONTROL | Historical attribution value for compatible response/token reads |

### Traffic rules

- traffic 0% assigns every request to HYBRID;
- traffic 100% assigns every eligible actor to ML_HYBRID;
- traffic between 0% and 100% uses deterministic bucketing;
- experiment disabled assigns HYBRID;
- ML disabled assigns HYBRID even when experiment traffic is configured;
- a valid model is still required before ML scores are used.
...
- rankingMode;
- rankingModelVersion;
- experiment ID and variant when applicable.

This makes it possible to verify whether a real request used Standard, valid AI-Hybrid, or fallback.
...
- candidate source configuration changes.
...
The response includes products, rank, score, source, reasons, strategy, profile state, pagination, request ID, policy version, ranking mode, model version, and experiment metadata.
...
| GET | /admin/recommendation/experiments | ADMIN_RECOMMENDATION_ANALYTICS_READ |
...
~~~json
{
  ...
  "reason": "Enable AI for a 20 percent rollout"
}
~~~
...
- trafficPercent is limited to 0 through 100;
- candidate flags must be booleans;
- fields not included in a patch keep their current values;
- infrastructure secrets, model paths, service URLs, and experiment ID remain environment-owned;
...
| RECOMMENDATION_WEIGHT_* | Interaction weights |
| RECOMMENDATION_HYBRID_RANKING_WEIGHT_* | Standard feature weights |
| RECOMMENDATION_TRACKING_SECRET | Signed attribution secret |
| RANKING_EXPERIMENT_ENABLED | Enable experiment assignment |
| RANKING_EXPERIMENT_ID | Stable experiment identifier |
| RANKING_EXPERIMENT_TRAFFIC_PERCENT | ML actor percentage |
...
Infrastructure credentials and model endpoints remain environment-owned. Admin policy can change ranking behavior, but it cannot change internal tokens, Qdrant connection details, model paths, or experiment identity.
...
13. Inspect response rankingMode, policy version, reasons, and experiment metadata.
14. Enable AI only after AI Service status and model readiness are confirmed.
...
- invalid blend and traffic are rejected;
- Standard cannot be disabled;
- candidate policy flags combine with environment master switches;
...
- AI rollout from 0% to a controlled percentage;
...
1. Deploy code with ML ranking disabled and AI traffic at 0%.
...
4. Enable the policy while keeping traffic at 0%.
5. Send controlled test requests and confirm rankingMode remains HYBRID.
6. Increase traffic to 5% or 10%.
7. Compare latency, fallback rate, click-through, conversion, and error metrics.
8. Increase gradually to 25%, 50%, and 100% only when metrics remain acceptable.
9. If the model is unhealthy, set traffic to 0% or disable ML.
10. Roll back the policy when a known previous configuration is safer.
...
4. set AI traffic to 0% if AI is involved;
5. verify HYBRID responses;
...
No. Standard Ranking is always the baseline and fallback. AI only contributes a bounded blend when policy, experiment assignment, model readiness, and response validation all succeed.

### Why does the response say HYBRID when AI is enabled in Admin?

The policy can be enabled while the model is unavailable, in fallback mode, timed out, or while the actor is outside the configured traffic bucket. Inspect rankingModelVersion and experiment metadata.
...
No. Those are infrastructure settings owned by deployment configuration. Admin controls ranking policy, rollout, and candidate-source flags only.
...
Designed the recommendation serving pipeline, profile projection, candidate sources, deterministic Standard Ranking, controlled AI rollout, attribution boundary, catalog intelligence, and admin policy workflow for the Bin E-Commerce ecosystem.
