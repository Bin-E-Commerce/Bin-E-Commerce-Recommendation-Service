...
export class AdminRecommendationService {
  ...
  private normalizeWeights(
    ...
    const unknown = Object.keys(values).filter((key) => !keys.includes(key));
    if (unknown.length > 0) {
      throw new BadRequestException(`Unknown ranking weight: ${unknown[0]}`);
    }
    const normalized = Object.fromEntries(
      keys.map((key) => {
        const value = Number(values[key] ?? 0);
        if (!Number.isFinite(value) || value < 0) {
          throw new BadRequestException(`Invalid ranking weight: ${key}`);
        }
        return [key, value];
      }),
    );
