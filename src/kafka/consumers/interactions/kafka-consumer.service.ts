...
import { KafkaEventProcessor } from '@/kafka/consumers/processors/kafka-event.processor';
import {
    validateCatalogEvent,
    validatePurchaseEvent,
} from '@/kafka/consumers/validators/event.validators';
import {
        ...
        try {
            ...
            await this.consumer.run({
                ...
                eachMessage: async ({ topic, partition, message }) => {
                    ...
                    if (
                        (
                            RECOMMENDATION_CATALOG_TOPICS as readonly string[]
                        ).includes(topic)
                    ) {
