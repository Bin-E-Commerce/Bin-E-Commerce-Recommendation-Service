    ...
    normalizeContent(
        ...
        return {
            ...
            shortDescription: input.shortDescription
                ? this.normalize(input.shortDescription, 1000)
                : null,
            description: input.description
                ? this.normalize(input.description, 4000)
                : null,
            brandName: input.brandName
                ? this.normalize(input.brandName, 255)
                : null,
            categoryPath: input.categoryPath
                ? this.normalize(input.categoryPath, 1000)
                : null,
            attributes,
            contentHash: '',
        };
    ...
    toEmbeddingText(content: RecommendationSemanticProductContent): string {
        const parts = [
            ...
            content.attributes.length
                ? `Attributes: ${content.attributes.map((item) => `${item.key}: ${item.value}`).join('; ')}`
                : '',
        ];
