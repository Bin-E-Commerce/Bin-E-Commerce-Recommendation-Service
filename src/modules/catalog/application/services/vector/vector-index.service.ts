...
export class VectorIndexService implements OnModuleInit, OnModuleDestroy {
    ...
    constructor(private readonly config: ConfigService) {
        ...
        this.aliasPromotionEnabled =
            config.get<string>('QDRANT_ALIAS_PROMOTION_ENABLED', 'false') ===
            'true';
        ...
        this.dimensions =
            Number.isInteger(configuredDimensions) && configuredDimensions > 0
                ...
                : 1536;
        const configuredDistance = config.get<string>(
            'QDRANT_DISTANCE',
            'Cosine',
        );
        this.distance =
    ...
    private async initializeCollection(): Promise<boolean> {
        ...
        try {
            ...
            if (!exists.ok) {
                ...
                if (!created.ok)
                    throw new Error(
                        `QDRANT_COLLECTION_CREATE_FAILED_${created.status}`,
                    );
            }
            ...
            const collectionInfo = await this.request(
                `/collections/${collection}`,
            );
            ...
            const vectorsConfig =
                collectionBody.result?.config?.params?.vectors;
            ...
            if (
                ...
                configuredSize !== this.dimensions
            ) {
                throw new Error(
            ...
            const canPromoteVersion =
                this.aliasPromotionEnabled && indexedPoints > 0;
            ...
            if (action) {
                ...
                if (!aliasUpdate.ok)
                    throw new Error(
                        `QDRANT_ALIAS_UPDATE_FAILED_${aliasUpdate.status}`,
                    );
            }
    ...
    async upsertProductVector(input: {
        ...
        isInStock: boolean;
        embeddingStatus?:
            'READY' | 'PENDING' | 'PROCESSING' | 'STALE' | 'FAILED';
    }): Promise<void> {
