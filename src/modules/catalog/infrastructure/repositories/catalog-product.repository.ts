...
export class CatalogProductRepository {
    ...
    async upsertIfNewer(
        ...
        const snapshot = {
            ...
            ...providedFields,
            catalogVersion:
                product.catalogVersion ?? current?.catalogVersion ?? '1',
            semanticAttributes:
    ...
    async findExplore(
        ...
        return this.findOrdered('product.product_id', 'ASC', limit, exclusions);
    }
    ...
    private applyShopExclusions(
        ...
        if (exclusions.excludeProductIds?.length) {
            query.andWhere(
                'product.product_id NOT IN (:...excludeProductIds)',
                {
