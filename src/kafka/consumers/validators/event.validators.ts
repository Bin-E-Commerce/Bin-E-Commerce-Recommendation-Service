...
import { InvalidKafkaEventError } from '@/kafka/consumers/processors/kafka-event.processor';
...
function requireEnvelope(
    ...
    if (
        ...
        !eventNames.has(event.eventName)
    ) {
...
export function validateCatalogEvent(
    ...
    for (const [
        index,
        attributeValue,
    ] of semanticContent.attributes.entries()) {
        if (
            !attributeValue ||
