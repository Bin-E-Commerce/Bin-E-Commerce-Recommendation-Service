...
export class RecommendationAdminRepository {
    ...
    async getOverview(from: Date, to: Date) {
        ...
        const [
            ...
            this.interactions
                ...
                .andWhere(
                    "cartInteraction.interaction_type = 'PRODUCT_ADDED_TO_CART'",
                )
                .getRawOne<CountRow>(),
            activityBase
                .clone()
        ...
        return {
            ...
            totals: {
                ...
                clickToCartRate:
                    clicks > 0
                        ? Number(
                              (
                                  Number(aggregate?.cartAdds ?? 0) / clicks
                              ).toFixed(4),
                          )
                        : 0,
            },
            ...
            topProducts: topProducts.map((row) => ({
                ...
                productName:
                    catalogByProductId.get(row.productId)?.name ?? null,
                imageUrl:
                    catalogByProductId.get(row.productId)?.imageUrl ?? null,
                originType:
                    catalogByProductId.get(row.productId)?.originType ?? null,
    ...
    async listActors(
        ...
        if (actorIds) {
            query.andWhere('interaction.user_id IN (:...actorIds)', {
                actorIds,
            });
        }
        ...
        const count = await this.interactions
            ...
            .andWhere(
                actorIds
                    ? 'countInteraction.user_id IN (:...actorIds)'
                    : '1 = 1',
                { actorIds },
            )
    ...
    async getUserActivity(
        ...
        const activityQuery = this.interactions
            ...
            .andWhere('interaction.recommendation_request_id IS NOT NULL')
            .andWhere(from ? 'interaction.occurred_at >= :from' : '1 = 1', {
                from,
            })
    ...
    async getRankingPerformance(from: Date, to: Date) {
        const rows = await this.interactions
            .createQueryBuilder('interaction')
            .select(
                'interaction.recommendation_experiment_variant',
                'rankingMode',
            )
            .addSelect('COUNT(*)', 'events')
            .addSelect(
            ...
            .andWhere('interaction.recommendation_request_id IS NOT NULL')
            .andWhere(
                'interaction.recommendation_experiment_variant IS NOT NULL',
            )
        ...
        return rows.map((row) => ({
            ...
            clickToCartRate:
                Number(row.clicks) > 0
                    ? Number(
                          (Number(row.cartAdds) / Number(row.clicks)).toFixed(
                              4,
                          ),
                      )
                    : 0,
