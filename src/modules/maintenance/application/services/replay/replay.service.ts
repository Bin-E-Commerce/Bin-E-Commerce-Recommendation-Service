...
import { CreateReplayJobDto } from '@/modules/maintenance/presentation/dto/create-replay-job.dto';
...
export class ReplayService implements OnModuleInit, OnModuleDestroy {
    ...
    async create(
        ...
        const eventIds = normalizedEvents.map(
        ...
        return this.repository.create(input.sourceTopic, normalizedEvents);
    }
    ...
    private async claimNextJob(): Promise<RecommendationReplayJobEntity | null> {
        return this.repository.claimNextJob(REPLAY_LEASE_MS);
    }
    ...
    private async processNext(): Promise<void> {
        ...
        if (!job) return;
        const events = await this.repository.findPendingEvents(job.jobId);
        if (events.length === 0) {
            await this.repository.markCompleted(job.jobId);
            return;
        }
        ...
        const maxAttempts =
            Number.isInteger(configuredMaxAttempts) && configuredMaxAttempts > 0
                ? Math.min(configuredMaxAttempts, 100)
            ...
            try {
                ...
                const lastError =
                    error instanceof Error
                        ? (error.message || error.name).slice(0, 255)
                        : 'PUBLISH_FAILED';
    ...
    private normalizeReplayEvent(
        ...
        if (
            ...
            throw new UnauthorizedException(
                'Replay event must contain eventName',
            );
        }
        return replayEvent;
    }
